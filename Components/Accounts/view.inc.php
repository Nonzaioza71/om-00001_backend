                    ...
                    <tr>
                         ...
                         <td>
                            <div class="btn-group gap-1 w-100 align-items-center">
                                ...
                                <button class="btn btn-warning" onclick="_handleUpdateUser('<?php echo $item['user_name']?>', <?php echo $item['user_id']?>, <?php echo $item['user_banned']?>)">
                                    แก้ไขข้อมูล
                                </button>
    ...
    function _handleBanUser(id, user_banned) {
        ...
        if(_confirm){
            fetch('Controllers/bannedUserByID.php', {
                ...
                method: "POST",
                body: JSON.stringify({ user_id: id, isBanned })
            })
            .then(res=>res.json())
            .then(data=>{
                console.log(data);
